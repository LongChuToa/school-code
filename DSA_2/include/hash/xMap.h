...
V xMap<K, V>::put(K key, V value) {
	...
	for (auto it = table[index].begin(); it != table[index].end(); it++) {
		if ((*it)->key == key && keyEQ((*it)->key, key)) {
			retValue = (*it)->value;
			(*it)->value = value;
			return retValue;
		}
...
bool xMap<K, V>::remove(K key, V value, void (*deleteKeyInMap)(K), void (*deleteValueInMap)(V)) {
	...
	int index = hashCode(key, capacity);
	for (auto it = table[index].begin(); it != table[index].end(); it++) {
		if (((*it)->key == key && keyEQ((*it)->key, key)) && ((*it)->value == value && valueEQ((*it)->value, value))) {
			if (deleteKeyInMap) {
				deleteKeyInMap((*it)->key);
			}
			if (deleteValueInMap) {
...
bool xMap<K, V>::containsValue(V value) {
	...
	for (int i = 0; i < count; i++) {
		for (auto it = table[i].begin(); it != table[i].end(); it++) {
			if ((*it)->value == value && valueEQ((*it)->value, value)) {
				return true;
			}
		}
	}
...
DLinkedList<int> xMap<K, V>::clashes() {
	//YOUR CODE IS HERE
	DLinkedList<int> clash;
	for (int i = 0; i < count; i++) {
		clash.add(table[i].size());
	}
	return clash;
}
